...
func aliasEncode(parameterStatus *parameterStatus, x interface{}, kbtypOid oid.Oid, allOid oid.AllOid) (value []byte) {
	...
	switch k {
		...
		case reflect.String:
			if allOid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(rv.String()))
			}else{ return []byte(rv.String()) }
		default:
...
func encode(parameterStatus *parameterStatus, x interface{}, kbtypOid oid.Oid, cn *conn) (value []byte) {
	...
	case string:
		if allOid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v))
		}else{ return []byte(v) }
	case VarChar:
		if allOid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v))
		}else{ return []byte(v) }
	...
	case sql.NullString:
		if allOid.T_bytea == kbtypOid { return encodeBytea(parameterStatus.serverVersion, []byte(v.String))
		}else{ return []byte(v.String) }
	case bool:
...
func binaryDecode(parameterStatus *parameterStatus, s []byte, typ oid.Oid, cn conn) (value interface{}) {
	...
	case cn.allOid.T_uuid:
		...
		value = b
		return
	default: errorf("don't know how to decode binary parameter of type %d", uint32(typ))
	}
	panic("not reached")
}
...
func appendEncodedText(parameterStatus *parameterStatus, buf []byte, x interface{}) (value []byte) {
	...
	case nil:
		value = append(buf, "\\N"...)
		return
	default: errorf("encode: unknown type for %T", v)
	}
	panic("not reached")
}
...
func appendEscapedText(buf []byte, text string) (value []byte) {
	...
	for i := startPos; len(text) > i; i++ {
		...
		case '\t': result = append(result, '\\', 't')
		default: result = append(result, c)
		}
	}
	value = result
	return
...
func mustParse(f string, typ oid.Oid, s []byte, cn conn) (pt time.Time) {
	...
	if nil != err { errorf("decode: %s", err) }
	return
}
...
func (p *timestampParser) expect(str string, char byte, pos int) (){
	...
	if c := str[pos]; char != c && nil == p.err { p.err = fmt.Errorf("expected '%v' at position %v; got '%v'", char, pos, c) }
}

func (p *timestampParser) mustAtoi(str string, begin int, end int) (result int) {
	if nil != p.err { return 0 }
	...
	if nil != err {
		if nil == p.err { p.err = fmt.Errorf("expected number; got '%v'", str) }
		result = 0
		return
	}
	return
...
func EnableInfinityTs(negative time.Time, positive time.Time) () {
	if infinityTsEnabled { panic(infinityTsEnabledAlready) }
	if !negative.Before(positive) { panic(infinityTsNegativeMustBeSmaller) }
...
func ParseTimestamp(currentLocation *time.Location, str string) (pt time.Time, err error) {
	...
	// Gregorian格式，不是ISO格式
	year := p.mustAtoi(str, 0, monSep); daySep := monSep + 3
	month := p.mustAtoi(str, monSep+1, daySep); p.expect(str, '-', daySep); timeSep := daySep + 3
	day := p.mustAtoi(str, daySep+1, timeSep)

	minLen, isBC := monSep + len("01-01") + 1, strings.HasSuffix(str, " BC")

	if isBC { minLen = minLen + 3 }

	var hour int
	var minute int
	var second int
	if minLen < len(str) {
		p.expect(str, ' ', timeSep); minSep := timeSep + 3
		p.expect(str, ':', minSep); hour = p.mustAtoi(str, timeSep+1, minSep); secSep := minSep + 3
		p.expect(str, ':', secSep); minute = p.mustAtoi(str, minSep+1, secSep)
		secEnd := secSep + 3; second = p.mustAtoi(str, secSep+1, secEnd)
	}
	...
	if len(str) > remainderIdx && '.' == str[remainderIdx] {
		...
		if 0 > fracOff { fracOff = len(str) - fracStart }
	...
	if tzStart := remainderIdx; len(str) > tzStart && ('-' == str[tzStart] || '+' == str[tzStart]) {
		...
		case '-': tzSign = -1
		case '+': tzSign = +1
		default: return time.Time{}, fmt.Errorf("expected '-' or '+' at position %v; got %v", tzStart, c)
		}
	...
	if len(str) > remainderIdx { return time.Time{}, fmt.Errorf("expected end of input, got %v", str[remainderIdx:]) }
...
func ParseTime(currentLocation *time.Location, str string) (pt time.Time, err error) {
	...
	p.expect(str, ':', minSep); hour = p.mustAtoi(str, 0, minSep); secSep := minSep + 3
	p.expect(str, ':', secSep); minute = p.mustAtoi(str, minSep+1, secSep); secEnd := secSep + 3
	second = p.mustAtoi(str, secSep+1, secEnd); remainderIdx := secEnd
...
func ParseDate(currentLocation *time.Location, str string) (dt time.Time, err error) {
	...
	isBC := strings.HasSuffix(str, " BC"); if isBC { minLen = minLen + 3 }

	var isoYear int

	if isBC { isoYear = 1 - year
	} else { isoYear = year }
...
func formatTs(t time.Time) (ts []byte) {
	if infinityTsEnabled {
		...
		if !t.After(infinityTsNegative) { return []byte("-infinity") }
		// t >= infinity : ! (!t < infinity)
		if !t.Before(infinityTsPositive) { return []byte("infinity") }
	}
...
func FormatTimestamp(t time.Time) (ts []byte) {
	...
	if 0 != offset {
		...
		b = append(b, ':')
		if 10 > offset { b = append(b, '0') }
		b = strconv.AppendInt(b, int64(offset), 10)
	}

	if true == bc { b = append(b, " BC"...) }
	ts = b
	return
}
	...
	if 2 <= len(s) && bytes.Equal(s[:2], []byte("\\x")) {
		...
		for 0 < len(s) {
			if '\\' == s[0] {
				...
				// '\\'后跟着一个八进制数
				if 4 > len(s) { return nil, fmt.Errorf("invalid bytea sequence %v", s) }
				r, err := strconv.ParseInt(string(s[1:4]), 8, 9)
				if nil != err { return nil, fmt.Errorf("could not parse bytea value: %s", err.Error()) }
				result = append(result, byte(r))
				s = s[4:]
			} else {
				// 遇到未转义字节时尝试尽可能多的读取
				i := bytes.IndexByte(s, '\\')
				if -1 == i { result = append(result, s...); break }
...
func encodeBytea(serverVersion int, v []byte) (result []byte) {
	if 90000 <= serverVersion {
		...
		for _, b := range v {
			if '\\' == b { result = append(result, '\\', '\\')
			} else if 0x20 > b || 0x7e < b { result = append(result, []byte(fmt.Sprintf("\\%03o", b))...)
			} else { result = append(result, b) }
		}
	}
	return
}
...
func (nt NullTime) Value() (dv driver.Value, err error) {
	if !nt.Valid { dv = nil; err = nil; return }
	dv = nt.Time
	err = nil
	return
}
