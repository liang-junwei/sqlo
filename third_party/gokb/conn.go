...
func (s transactionStatus) String() (result string) {
	...
	case txnStatusInFailedTransaction:
		...
		return
	default: errorf("unknown transactionStatus %d", s)
	}
	panic("not reached")
}
...
func (cn *conn) handleDriverSettings(o values) (err error) {
	boolSetting := func(key string, val *bool) (fErr error) {
		if value, ok := o[key]; ok {
			if "yes" == value { *val = true
			} else if "no" == value { *val = false
			} else {
...
func (cn *conn) handleKbpass(o values) () {
	...
	for scanner.Scan() {
		line := scanner.Text()
		if 0 == len(line) || '#' == line[0] { continue }
		split := getFields(line)
		if 5 != len(split) { continue }
...
func (c *Connector) open(ctx context.Context) (cn *conn, err error) {
	...
	// 重置deadline
	if timeout, ok := o["connect_timeout"]; "0" != timeout && ok { err = cn.c.SetDeadline(time.Time{}) }
...
func getPrimary(cn *conn) (isPrimary bool, err error) {
	...
	for {
		...
		switch t {
		case 'E': err = parseError(&rs.rb)
		case 'C', 'I', 'Z':
			if 'C' == t { rs.result, rs.tag = cn.parseComplete(rs.rb.string(), 0) }
			break
		case 'T':
		...
		case 'D':
			...
			continue
		default: errorf("unexpected message after execute: %q", t)
		}
	}
...
func dial(ctx context.Context, d Dialer, o values) (nc net.Conn, err error) {
	...
	if "unix" == network { o["sslmode"] = "disable" }
	// 0或者不设置表示无限期等待
...
func (cn *conn) rollback() (err error) {
	...
	if nil != err {
		if cn.isInTransaction() { cn.bad = true }
		return
	}
...
func (cn *conn) simpleExec(q string) (res driver.Result, commandTag string, err error) {
	...
	for {
		...
		case 'I': res = emptyRows
		case 'E': err = parseError(r)
		default:
			cn.bad = true
...
func (cn *conn) saveMessage(typ byte, buf *readBuf) () {
	if 0 != cn.saveMessageType { cn.bad = true;
...
func (cn *conn) recvMessage(rb *readBuf) (result byte, err error) {
	...
	var y []byte
	if len(cn.scratch) >= n { y = cn.scratch[:n]
	} else { y = make([]byte, n) }
...
func (cn *conn) recv() (t byte, rb *readBuf) {
	for {
		...
		if nil != err { panic(err) }
		switch t {
		case 'E': panic(parseError(rb))
		case 'N':
			if n := cn.noticeHandler; nil != n { n(parseError(rb)) }
		default: return
		}
	}
}
...
func (cn *conn) recv1Buf(r *readBuf) (result byte) {
	for {
		...
		case 'A':
		case 'N':
			if n := cn.noticeHandler; nil != n { n(parseError(r)) }
		case 'S': cn.processParameterStatus(r);
		default:
			result = t
			return
		}
...
func (cn *conn) ssl(o values) (err error) {
	...
	w := cn.writeBuf(0); w.int32(80877103)
	if err = cn.sendStartupPacket(w); nil != err { return }
...
func isDriverSetting(key string) (state bool) {
	switch key {
	case "host", "port": fallthrough
	case "password": fallthrough
	case "sslmode", "sslcert", "sslkey", "sslrootcert": fallthrough
	case "fallback_application_name": fallthrough
	case "connect_timeout": fallthrough
	//case "keepalive_idle": fallthrough//go中idle和interval为相同值
	case "keepalive_interval": fallthrough
	case "keepalive_count": fallthrough
	case "tcp_user_timeout": fallthrough
	case "get_last_insert_id": fallthrough
	case "disable_prepared_binary_result": fallthrough
	case "binary_parameters": fallthrough
	case "target_session_attrs": fallthrough
	case "retry": fallthrough
	case "delay": state = true
	default: state = false
	}
	return
}
...
func (cn *conn) startup(value values) () {
	...
	for k, v := range value {
		// 跳过非规定的连接参数
		if isDriverSetting(k) { continue; }
		// 协议规定数据库名为database而不是dbname
		if "dbname" == k { k = "database"; }
		w.string(k); w.string(v)
	}
	w.string("")
	if err := cn.sendStartupPacket(w); nil != err { panic(err); }

	for {
		t, r := cn.recv()
		switch t {
		case 'K': cn.processBackendKeyData(r)
		case 'S': cn.processParameterStatus(r)
		case 'R': cn.auth(r, value)
		case 'Z': cn.processReadyForQuery(r)
			return
		default: errorf("unknown response for startup: %q", t)
		}
	}
}
...
func (cn *conn) auth(r *readBuf, o values) () {
	...
	case 3:
		w := cn.writeBuf('p')
		w.string(o["password"]); cn.send(w)

		t, r := cn.recv()
		if 'R' != t { errorf("unexpected password response: %q", t) }

		if 0 != r.int32() { errorf("unexpected authentication response: %q", t) }
	case 5:
	...
	case 10:
		...
		sc.Step(nil)
		if nil != sc.Err() { errorf("SCRAM-SHA-256 error: %s", sc.Err().Error()) }
		scOut := sc.Out()

		w := cn.writeBuf('p')
		w.string("SCRAM-SHA-256"); w.int32(len(scOut)); w.bytes(scOut)
		cn.send(w)

		t, r := cn.recv()
		if 'R' != t { errorf("unexpected password response: %q", t) }

		if 11 != r.int32() { errorf("unexpected authentication response: %q", t) }

		nextStep := r.next(len(*r))
		sc.Step(nextStep)
		if nil != sc.Err() { errorf("SCRAM-SHA-256 error: %s", sc.Err().Error()) }

		scOut = sc.Out(); w = cn.writeBuf('p')
		w.bytes(scOut); cn.send(w)

		t, r = cn.recv()
		if 'R' != t { errorf("unexpected password response: %q", t) }

		if 12 != r.int32() { errorf("unexpected authentication response: %q", t) }
	...
	default: errorf("unknown authentication response: %d", code)
	}
}
...
func (st *stmt) Close() (err error) {
	...
	w := st.cn.writeBuf('C')
	w.byte('S'); w.string(st.name)
	st.cn.send(w); st.cn.send(st.cn.writeBuf('S'))

	t, _ := st.cn.recv1()
	if '3' != t { st.cn.bad = true;
		errorf("unexpected close response: %q", t)
	}
	st.closed = true

	t, r := st.cn.recv1()
	if 'Z' != t { st.cn.bad = true;
...
func (st *stmt) exec(v []driver.Value) (err error) {
	...
	if 65536 <= len(v) { return fmt.Errorf("got %d parameters but Kingbase only supports 65535 parameters", len(v)) }
	...
	cn := st.cn; w := cn.writeBuf('B')
	w.byte(0); w.string(st.name)

	if cn.binaryParameters { cn.sendBinaryParameters(w, v);
	} else {
		w.int16(len(v))
		for i := 0; i < len(v); i++ {
		...
		for i, x := range v {
			if nil == x { w.int32(-1);
			}  else { b := encode(&cn.parameterStatus, x, st.paramTyps[i], cn);
				if b != nil {
					w.int32(len(b))
					w.bytes(b)
				} else {
...
func (cn *conn) parseComplete(commandTag string, lastInsertId int64) (result driver.Result, s string) {
	...
	for _, tag := range commandsWithAffectedRows {
		if strings.HasPrefix(commandTag, tag) { t := commandTag[len(tag):];
	...
	if nil == affectedRows && strings.HasPrefix(commandTag, "INSERT ") {
		...
		if 3 != len(parts) { cn.bad = true;
			errorf("unexpected INSERT command tag %s", commandTag)
		}
		affectedRows, commandTag = &parts[len(parts)-1], "INSERT"
	}
	// 理应没有被影响的行，直接返回
	if nil == affectedRows { return driver.RowsAffected(0), commandTag }
...
func (rs *rows) Close() (err error) {
	if finish := rs.finish; nil != finish { defer finish() }
	// 将会检查cn.bad，此处不需要处理
	for {
		err = rs.Next(nil)
		switch err {
...
func (rs *rows) Next(dest []driver.Value) (err error) {
	...
	for {
		...
		case 'E':
			err = parseError(&rs.rb)
		case 'C', 'I':
			if 'C' == t { rs.result, rs.tag = conn.parseComplete(rs.rb.string(), 0) }
		...
		case 'D':
			if rs.outParamInMultiRes {
				...
				//DQL的结果集
				n := rs.rb.int16()
				if nil != err { conn.bad = true;
					errorf("unexpected DataRow after error %s", err)
				}
				if len(dest) > n { dest = dest[:n] }
				for i := range dest {
					l := rs.rb.int32()
					if -1 == l {
						// 空对象
...
func QuoteLiteral(literal string) (s string) {
	...
	if strings.Contains(literal, `\`) { literal = strings.Replace(literal, `\`, `\\`, -1);
		literal = ` E'` + literal + `'`
...
func (cn *conn) sendBinaryParameters(b *writeBuf, args []driver.Value) () {
	...
	for i, x := range args {
		_, ok := x.([]byte)
		if ok {
			if nil == paramFormats { paramFormats = make([]int, len(args)) }
			paramFormats[i] = 1
		}
	}
	if nil == paramFormats { b.int16(0)
	} else {
		b.int16(len(paramFormats))
		for _, x := range paramFormats { b.int16(x) }
	}
...
func (cn *conn) processParameterStatus(rb *readBuf) () {
	...
	case "server_version":
		...
		if nil == err { cn.parameterStatus.serverVersion = major1*10000 + major2*100 + minor }
	case "TimeZone":
		cn.parameterStatus.currentLocation, err = time.LoadLocation(rb.string())
		if nil != err { cn.parameterStatus.currentLocation = nil }
	default:
		// ignore
	}
	return
...
func (cn *conn) sendBinaryModeQuery(query string, args []driver.Value) () {
	if 65536 <= len(args) { errorf("got %d parameters but Kingbase only supports 65535 parameters", len(args)) }
...
func (cn *conn) readStatementDescribeResponse() (paramTyps []oid.Oid, colNames []string, colTyps []fieldDesc, err error) {
	...
	for {
		...
		case 't':
			...
			for i := range paramTyps { paramTyps[i] = r.oid() }
		case 'T':
...
func (cn *conn) readExecuteResponse(protocolState string, v []driver.Value, colTyps []fieldDesc, colFmts []format) (res driver.Result, commandTag string, err error) {
	...
	for {
		...
		case 'Z':
			cn.processReadyForQuery(r)
			if nil == res && nil == err { err = errUnexpectedReady }
...
func parseEnviron(env []string) (out map[string]string) {
	...
	for _, v := range env {
		...
		switch parts[0] {
		case "KINGBASE_HOST": accrue("host")
		case "KINGBASE_PORT": accrue("port")
		case "KINGBASE_DATABASE": accrue("dbname")
		case "KINGBASE_USER": accrue("user")
		case "KINGBASE_PASSWORD": accrue("password")
		case "KINGBASE_OPTIONS": accrue("options")
		case "KINGBASE_APPNAME": accrue("application_name")
		case "KINGBASE_SSLMODE": accrue("sslmode")
		case "KINGBASE_SSLCERT": accrue("sslcert")
		case "KINGBASE_SSLKEY": accrue("sslkey")
		case "KINGBASE_SSLROOTCERT": accrue("sslrootcert")
		case "KINGBASE_CONNECT_TIMEOUT": accrue("connect_timeout")
		//case "KINGBASE_KEEPALIVE_IDLE": accrue("keepalive_idle")//go中idle和interval为相同值
		case "KINGBASE_KEEPALIVE_INTERVAL": accrue("keepalive_interval")
		case "KINGBASE_KEEPALIVE_COUNT": accrue("keepalive_count")
		case "KINGBASE_TCP_USER_TIMEOUT": accrue("tcp_user_timeout")
		case "KINGBASE_GET_AUTO_INCREMENT_ID": accrue("get_auto_increment_id")
		case "KINGBASE_CLIENTENCODING": accrue("client_encoding")
		case "KINGBASE_DATESTYLE": accrue("datestyle")
		case "KCITZ": accrue("timezone")
		case "KCIGEQO": accrue("geqo")
		case "KINGBASE_HOSTADDR": fallthrough
		case "KINGBASE_SERVICE", "KINGBASE_SERVICEFILE", "KINGBASE_REALM": fallthrough
		case "KINGBASE_REQUIRESSL", "KINGBASE_SSLCRL": fallthrough
		case "KINGBASE_REQUIREPEER": fallthrough
		case "KINGBASE_KRBSRVNAME", "KINGBASE_GSSLIB": fallthrough
		case "KINGBASE_SYSCONFDIR", "KINGBASE_LOCALEDIR": unsupported()
		}
	}
	return
}
